...
int roi_align_rotated_backward_cpu(at::Tensor top_grad, at::Tensor rois,
                            int pooled_height, int pooled_width,
                            float spatial_scale, int sample_num,
                            at::Tensor bottom_grad) {
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("forward", &roi_align_rotated_forward_cpu, "Roi_Align_Rotated forward (cpu)");
  m.def("backward", &roi_align_rotated_backward_cpu, "Roi_Align_Rotated backward (cpu)");
}
